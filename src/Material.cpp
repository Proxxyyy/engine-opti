...
namespace OM3D
{
    ...
    void Material::set_program(std::shared_ptr<Program> prog) { _program = std::move(prog); }

    void Material::set_blend_mode(BlendMode blend) { _blend_mode = blend; }

    void Material::set_depth_test_mode(DepthTestMode depth) { _depth_test_mode = depth; }

    void Material::set_depth_write(bool write) { _depth_write = write; }

    void Material::set_double_sided(bool double_sided) { _double_sided = double_sided; }

    void Material::set_texture(u32 slot, std::shared_ptr<Texture> tex)
    {
        if (const auto it =
                    std::find_if(_textures.begin(), _textures.end(), [&](const auto& t) { return t.first == slot; });
            it != _textures.end())
        {
    ...
    bool Material::is_opaque() const { return _blend_mode == BlendMode::None; }
