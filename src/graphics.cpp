...
namespace OM3D
{
    ...
    void debug_out(GLenum, GLenum type, GLuint, GLenum sev, GLsizei, const char* msg, const void*)
    {
        ...
        (is_error ? std::cerr : std::cout)
                << (sev == GL_DEBUG_SEVERITY_HIGH ? "[GL][HIGH] " : "[GL] ") << msg << std::endl;
    ...
    bool bindless_enabled() { return GLAD_GL_ARB_bindless_texture != 0; }

    void init_graphics()
    {
        ALWAYS_ASSERT(gladLoadGL(glfwGetProcAddress), "glad initialization failed");

        std::cout << "OpenGL " << glGetString(GL_VERSION) << " initialized on " << glGetString(GL_VENDOR) << " "
                  << glGetString(GL_RENDERER) << " using GLSL " << glGetString(GL_SHADING_LANGUAGE_VERSION)
                  << std::endl;
    ...
    const Texture& brdf_lut() { return brdf_lut_texture; }
    ...
    std::shared_ptr<Texture> default_black_texture() { return default_textures.black; }

    std::shared_ptr<Texture> default_white_texture() { return default_textures.white; }

    std::shared_ptr<Texture> default_normal_texture() { return default_textures.normal; }

    std::shared_ptr<Texture> default_metal_rough_texture() { return default_textures.metal_rough; }
...

        {
            ...
            for (int i = 0; i != uniform_count; ++i)
            {
                ...
                if (is_sampler_type(type) || is_image_type(type))
                {
                    ...
                    ALWAYS_ASSERT(
                            glIsTexture(get_at(is_cube_type(type) ? GL_TEXTURE_BINDING_CUBE_MAP : GL_TEXTURE_BINDING_2D,
                                               index)),
                            "Bound texture is destroyed or invalid");
                }
            }
        }
