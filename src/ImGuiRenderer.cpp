...
namespace OM3D
{
    ...
    static std::unique_ptr<Texture> create_font()
    {
        ...
        fonts->AddFontFromMemoryCompressedTTF(font_awesome_compressed_data, font_awesome_compressed_size, 13.0f,
                                              &config, icon_ranges);
    ...
    static void char_callback(GLFWwindow*, unsigned characted) { ImGui::GetIO().AddInputCharacter(characted); }
    ...
    void ImGuiRenderer::render(const ImDrawData* draw_data)
    {
        ...
        for (int c = 0; c != draw_data->CmdListsCount; ++c)
        {
            ...
            for (int i = 0; i != cmd_list->CmdBuffer.Size; ++i)
            {
                ...
                glScissor(int(clip_min.x), int(height - clip_max.y), int(clip_max.x - clip_min.x),
                ...
                glVertexAttribPointer(2, 4, GL_UNSIGNED_BYTE, false, sizeof(ImDrawVert),
                ...
                glDrawElements(GL_TRIANGLES, cmd.ElemCount,
                               ...
                               reinterpret_cast<void*>(drawn_index_offset));
